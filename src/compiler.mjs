...
export function buildTrace(program, inputs = {}) {
  const { steps, registerCount, memoryRegisterCount, opcodeRegisterCount } =
    program;
  const trace = [];
  ...
  for (const { opcode, name, args } of steps) {
    ...
    if (name === "set") {
      ...
      outputSelector[+args[0]] = 1n;
      memoryRegisters[+args[0]] = field[name](
        memoryRegisters[+args[1]],
        memoryRegisters[+args[2]]
      );
...
export function compile(asm) {
  const steps = asm
    ...
    .map((operation, i) => {
      ...
      const [op, ...args] =
        commentIndex >= 0 ? operation.slice(0, commentIndex) : operation;
      ...
      if (argumentCount !== args.length) {
        throw new Error(
          `Invalid number of arguments for "${op}" on line ${i}. Expected ${argumentCount}, received ${args.length}`
        );
      }
      return {
        opcode,
  ...
  for (
    let x = memoryRegisterCount;
    x < memoryRegisterCount * 4 + opcodeRegisterCount;
    x++
  ) {
  ...
  for (
    let x = 4 * memoryRegisterCount;
    x < 4 * memoryRegisterCount + opcodeRegisterCount;
    x++
  ) {
  ...
  const freeInRegister =
    prevState[4 * memoryRegisterCount + opcodeRegisterCount];
  ...
  const outputEqualityConstraint = new MultiPolynomial(field);
  const outputConstraint = new MultiPolynomial(field);
  for (let x = 0; x < memoryRegisterCount; x++) {
    outputConstraint.add(
      nextState[x].copy().mul(prevState[memoryRegisterCount + x])
    );
    outputEqualityConstraint.add(prevState[x].copy().sub(nextState[x]));
    read1Constraint.add(
      prevState[x].copy().mul(prevState[2 * memoryRegisterCount + x])
    );
    read2Constraint.add(
      prevState[x].copy().mul(prevState[3 * memoryRegisterCount + x])
    );
  }
  ...
  constraints.push(
    set
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["set"].opcode])
  );
  ...
  constraints.push(
    add
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["add"].opcode])
  );
  ...
  constraints.push(
    neg
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["neg"].opcode])
  );
  ...
  constraints.push(
    mul
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["mul"].opcode])
  );
  ...
  constraints.push(
    eq
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["eq"].opcode])
  );

  // for the equality operation constrain that the output doesn't change
  // because we're not using it
  const eq2 = outputEqualityConstraint;
  constraints.push(
    eq2
      .copy()
      .mul(prevState[4 * memoryRegisterCount + validOperations["eq"].opcode])
